...
const ValidatorCategory = require(__base__validator + "category");
const CategoryModel = require(__base__model + "category");
...
const model = require(__base__model + 'category');

const link = `/${systemConfig.prefixAdmin}/category/status/`;
const titlePage = 'CATEGORY - ';
const titleList = titlePage + "LIST";
const titleADD = titlePage + "ADD";
const titleEDIT = titlePage + "EDIT";
const folderView = "admin/category/";
...
router.get('/status/(:status)?', async (req, res, next) => {
  ...
  let statusFillter = await UtilsHelper.createFillterStatus(params.currentStatus, 'category');

  await CategoryModel.countItems({}).then((data) => {
    params.pagination.totalItem = data;
  });

  await CategoryModel.listItems(params)
    .then((data) => {
...
router.get("/change-status/:id/:status", (req, res, next) => {
  ...
  CategoryModel.changStatus(id, curentStatus, '', { task: "change-status" }).then((result) => {
...
router.post("/change-status/:status", async (req, res, next) => {
  ...
  await CategoryModel.changStatus('', curentStatus, id_changeMulti, { task: "change-status-multi" }).then((result) => {
...
router.get("/delete/:id", (req, res, next) => {
  ...
  CategoryModel.deleteItem(id).then((result) => {
...
router.post("/change-ordering/", async (req, res, next) => {
  ...
  await CategoryModel.changOrdering(cids, currentOrdering).then((result) => {
...
router.get('/form(/:id)?', function (req, res, next) {
  ...
  if (id === "") {//category ADD
    ...
    CategoryModel.getItem(id).then((result) => {
...
//ADD & EDIT
router.post('/form/save', ValidatorCategory.validator(), (req, res, next) => {
  ...
  if (item.id !== "") {//EDIT ITEM
    ...
    if (!errors.isEmpty()) {
      res.render("category/form", { errors, item, title: "ITEM EDIT" })// cho nay phai render day du cac thuoc tinh phia tren SHOW ADD VS EDIT
    } else {
      CategoryModel.editItem(item, { title: "ITEM EDIT" }).then((result) => {
      })
    ...
    if (!errors.isEmpty()) {
      ...
      CategoryModel.addItem(req.body).then((data) => {
      })
